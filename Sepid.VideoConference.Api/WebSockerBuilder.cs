    ...
    public static class WebSocketBuilder
    {
        ...
        private static async Task Stream(HttpContext context, WebSocket webSocket)
        {
            ...
            while (!result.CloseStatus.HasValue)
            {
                ...
                await using (var ms1 = new MemoryStream(imageData))
                {
                    var ss = Image.FromStream(ms1) as Bitmap;
                ...
                await webSocket.SendAsync(new ArraySegment<byte>(modifiedData, 0, modifiedData.Length),
